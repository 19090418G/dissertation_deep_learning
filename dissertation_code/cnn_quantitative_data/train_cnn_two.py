...
model = Sequential()
model.add(Conv1D(filters=64, kernel_size=3, activation='tanh', input_shape=(70, 1), kernel_regularizer=l2(0.001)))
model.add(BatchNormalization())
model.add(MaxPooling1D(pool_size=2))

model.add(Conv1D(filters=128, kernel_size=3, activation='tanh', kernel_regularizer=l2(0.001)))
model.add(BatchNormalization())
model.add(MaxPooling1D(pool_size=2))

model.add(Conv1D(filters=256, kernel_size=3, activation='tanh', kernel_regularizer=l2(0.001)))
model.add(BatchNormalization())
model.add(MaxPooling1D(pool_size=2))

model.add(Conv1D(filters=512, kernel_size=3, activation='tanh', kernel_regularizer=l2(0.001)))
model.add(BatchNormalization())
model.add(MaxPooling1D(pool_size=2))
...
model.fit(x_train, y_train, validation_data=(x_val, y_val), verbose=2, epochs=100, shuffle=False, batch_size=256, callbacks=[
    CSVLogger('model_two.log'),
    ReduceLROnPlateau(factor=0.8, patience=20, verbose=2),
    ModelCheckpoint('model_two.h5', save_best_only=True, monitor='val_acc', verbose=2)
])
